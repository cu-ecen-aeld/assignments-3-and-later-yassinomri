...
{
    const char *expected_username = "yassinomri";
    const char *conf_username = malloc_username_from_conf_file();
    const char *my_username_result = my_username();

    TEST_ASSERT_EQUAL_STRING_MESSAGE(expected_username, conf_username, "Username from conf file does not match");
    TEST_ASSERT_EQUAL_STRING_MESSAGE(expected_username, my_username_result, "Hardcoded username does not match");
